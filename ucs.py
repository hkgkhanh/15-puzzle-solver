...
def solve_ucs(initial_board, goal_state, update_gui_callback):
    """Solves the 15-puzzle using Uniform Cost Search and updates the GUI."""
    ...
    initial_state = (0, initial_board, [])  # (cost, board, path_to_here)
    priority_queue = [initial_state]  # Priority queue (heap)
    ...
    def ucs_search():
        while priority_queue:
            ...
            if board_to_tuple(current_board) == goal_tuple:
                print(f"✅ Solution found! Moves: {path}, Cost: {cost}")
            ...
            for neighbor_board, move in get_neighbors(current_board):
                ...
                if neighbor_tuple not in visited:
                    ...
                    heapq.heappush(priority_queue, (new_cost, neighbor_board, path + [move]))

        print("No solution found.")
        return []

    search_thread = threading.Thread(target=ucs_search)
    search_thread.start()